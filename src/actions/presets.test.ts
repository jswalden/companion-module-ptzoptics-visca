...
describe('preset upgrading of non-preset action', () => {
	describe('test preset upgrading of non-preset action', () => {
		test('non-upgradable action', async () => {
			...
			expect(action.actionId).toBe('foobar')
			expect(action.options[PresetValueOptionId]).toBe('123')
		})
	})
})
...
describe('obsolete preset recall upgrades', () => {
	test('upgradable with constant preset', async () => {
		...
		expect(action.actionId).toBe(PresetActionId.RecallPset)
		expect(action.options[PresetUseVariablesOptionId]).toBe(false)
		expect(action.options[PresetValueOptionId]).toBe('42')
		expect(action.options[PresetVariableOptionId]).toBe(`66`)
	})
	...
	test('upgradable with variable preset containing number', async () => {
		...
		expect(tryUpdateRecallSetPresetActions(action)).toBe(true)
		expect(action.actionId).toBe(PresetActionId.RecallPset)
		expect(action.options[PresetUseVariablesOptionId]).toBe(true)
		expect(action.options[PresetValueOptionId]).toBe(twoDigitHex(42))
		expect(action.options[PresetVariableOptionId]).toBe('42')
	})
	...
	test('upgradable with variable preset containing variable', async () => {
		...
		expect(action.actionId).toBe(PresetActionId.RecallPset)
		expect(action.options[PresetUseVariablesOptionId]).toBe(true)
		expect(action.options[PresetValueOptionId]).toBe(twoDigitHex(PresetRecallDefault))
		expect(action.options[PresetVariableOptionId]).toBe('1$(internal:custom_var)')
	})
})
...
describe('obsolete preset save upgrades', () => {
	test('upgradable with constant preset', async () => {
		...
		expect(tryUpdateRecallSetPresetActions(action)).toBe(true)
		expect(action.actionId).toBe(PresetActionId.SavePset)
		expect(action.options[PresetUseVariablesOptionId]).toBe(false)
		expect(action.options[PresetValueOptionId]).toBe('42')
		expect(action.options[PresetVariableOptionId]).toBe('66')
	})
	...
	test('upgradable with variable preset containing variable', async () => {
		...
		expect(action.actionId).toBe(PresetActionId.SavePset)
		expect(action.options[PresetUseVariablesOptionId]).toBe(true)
		expect(action.options[PresetValueOptionId]).toBe(twoDigitHex(PresetSetDefault))
		expect(action.options[PresetVariableOptionId]).toBe('1$(internal:custom_var)')
	})
})
